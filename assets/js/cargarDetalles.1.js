...
function getFavorito(usuario, pelicula){
	...
	peticion_httpFav.send("usuario="+usuario+"&pelicula="+pelicula);
...
function getEstado(usuario, pelicula){
	if(window.XMLHttpRequest){
	...
	peticion_httpEstado.send("usuario="+usuario+"&pelicula="+pelicula);
...
function cambiarIconoYEstado(usuario, peli){
	cambiarIcono();
	enviarAjaxFav(usuario, peli);
}
...
function enviarAjaxFav(usuario, peli){
	if(window.XMLHttpRequest){
		peticion_http= new XMLHttpRequest();
	}else if (window.ActiveXObject) {
		peticion_http= new ActiveXObject("Microsoft.XMLHTTP");
	...
	peticion_http.setRequestHeader("Content-Type", "application/x-www-form-urlencoded");
	peticion_http.send("usuario="+usuario+"&peli="+peli);
	peticion_http.onreadystatechange=muestraContenido;
}
...
function valorar(est, valor, usuario, pelicula){
	if(valor==1){
	...
	if(est==true){
		enviarValoracion(usuario, pelicula, valor);
	}
}
//Enviar mediante AJAX la valoración de una película
function enviarValoracion(usuario, pelicula, valor){
	if(window.XMLHttpRequest){
		peticion_http= new XMLHttpRequest();
	}else if (window.ActiveXObject) {
		peticion_http= new ActiveXObject("Microsoft.XMLHTTP");
	...
	peticion_http.setRequestHeader("Content-Type", "application/x-www-form-urlencoded");
	peticion_http.send("usuario="+usuario+"&pelicula="+pelicula+"&valoracion="+valor);
	peticion_http.onreadystatechange=muestraContenido;
}
//Coger la valoración de una película de la base de datos
function getValoracion(usuario, pelicula){
	if(window.XMLHttpRequest){
	...
	peticion_httpValoracion.send("usuario="+usuario+"&pelicula="+pelicula);
